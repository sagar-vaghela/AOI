...
import { getManageConfigQAMTable, newDataBase } from "../../actions/manageCongigQAM";
...
export default function ManageConfigurationPage({ setActiveTab }) {
  const mcQAMTableUser = useSelector(
    (state) => state.manageConfigQAMReducer.mcQAMTableUser.data
  );
  const mcQAMTableSystem = useSelector(
    (state) => state.manageConfigQAMReducer.mcQAMTableSystem.data
  );
  ...
  useEffect(() => {
    let newdata = [];
    if (mcQAMTableSystem) {
      mcQAMTableSystem.map((data) => {
        newdata.push({ name: data, editable: "no" });
      });
    }
    if (mcQAMTableUser) {
      mcQAMTableUser.map((data) => {
        newdata.push({ name: data, editable: "yes" });
      });
    }
    setTableData(newdata);
  }, [mcQAMTableSystem, mcQAMTableUser]);
      ...
      <div className="action mb-4 border border-dark p-2">
        ...
        <div className="action_btns justify-content-between">
          <div className="left_btns">
            ...
            />
            <Button label={"Edit"} handleClick={() => {setActiveTab("configuration")}} />
          </div>
