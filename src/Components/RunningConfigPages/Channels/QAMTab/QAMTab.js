...
import {
  ...
  postSaveAs,
  showPopUp,
} from "../../../../actions/runConfigQAM";
import { useSelector, useDispatch } from "react-redux";

export default function QAMTab(props) {
  const dispatch = useDispatch();

  const rcQAMTableData = useSelector(
    (state) => state.runConfigQAMReducer.rcQAMTable.data
  );
  ...
  const editHandleClick = () => {
    const selectRowLength = document.querySelectorAll(
      "#running_qam_table .selection-row"
    ).length;
  ...
  const defaultHandleClick = () => {
    ...
    dispatch(
      showPopUp({
        message: "Action was complete successfully!",
        type: "success",
      })
    );
  };
  ...
  const selectRow = {
    ...
    onSelect: (row, isSelected, rowIndex, e) =>
      console.log("onSelect====", row, isSelected, rowIndex, e),
  };
